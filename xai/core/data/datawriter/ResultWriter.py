...
class ResultWriter(object):
    ...
    def result_file_write(**kwargs):
        ...
        # Common.LOGGER.get_logger().info("[{}] : {} rows".format(file_name, len_results))
        if len_results == 0:
            return
        ...
        while start <= len_results:
            ...
            # Common.LOGGER.get_logger().info("[{}_{}] : {} rows".format(file_name, idx, len(batch_result)))
            ...
            # Common.LOGGER.get_logger().info("{} result file write complete - {}_{}.{}".format(ext, file_name, idx, "done"))
            start += batch_size
            idx += 1
